...
int create_database(Dlist *head,M_node **d_base)
{
    ...
    while(h_temp != NULL)
    {
	strcpy(temp,h_temp->file_name);                
	FILE *fptr = fopen(h_temp->file_name,"r");     // Opening file
	while(fscanf(fptr,"%s",buff) != EOF)           // looping for getting each word from file to EOF
	{
	    index = tolower(buff[0]) % 97;             // Defining index value of first letter for lower or upper both case
		...
		while(m_temp != NULL)
		{
			...
			/* file name of the word is already present in the Table */
			while(s_temp != NULL)
			{
			...
			{
			    flag = 0;   // using flag to declare the word or file names is present or not
			    break;
			}
		    }
